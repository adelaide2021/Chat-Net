...
/*
 把join and leave的消息放到队列中并处理的handler？？？
 */
@Slf4j
public class JoinAndLeaveHandler implements Handler<JoinAndLeaveMessage> {
   ...
   * no usage你怕不是在逗我？？？根本没有使用过这个方程，那么应该在哪里使用啊？？？
   * 不需要用是因为两阶段transaction的时候就直接附带了这个，已经执行了！如果惹refactor的话是可以讲transaction转到join and leave handler中来执行，这样就更加解耦
   * */
        ...
        if (ClientData.myNode.isLeader()) {
          if (ClientData.clientLiveNodes.size() != 0) {
            ...
            try {
              ...
              // report to server
              ClientData.centralServer.writeAndFlush(new JoinAndLeaveMessage(MessageType.JOIN_AND_LEAVE, JoinAndLeaveMessageType.JOIN, joinAndLeaveMessage.getNodeInfo()));
      ...
      case LEAVE:
        if (ClientData.myNode.isLeader()) {
          ...
          if (ClientData.clientLiveNodes.size() > 1) {
            ...
            // report to server
            ClientData.centralServer.writeAndFlush(new JoinAndLeaveMessage(MessageType.JOIN_AND_LEAVE, JoinAndLeaveMessageType.LEAVE, joinAndLeaveMessage.getNodeInfo()));
          }
        }
        break;
      case GREETING:
